...
test('UI or handler cannot jump directly to VERIFIED', () => {
  assert.throws(
    () => assertTaskTransition('READY', 'VERIFIED'),
    (error: any) => error.code === 'INVALID_TASK_TRANSITION',
  );
  assert.throws(
    () => assertTaskTransition('AWAITING_EVIDENCE', 'VERIFIED'),
    (error: any) => error.code === 'INVALID_TASK_TRANSITION',
  );
});
...
test('completed runs are immutable terminal history', () => {
  assert.throws(
    () => assertRunTransition('COMPLETED', 'VERIFYING'),
    (error: any) => error.code === 'INVALID_RUN_TRANSITION',
  );
