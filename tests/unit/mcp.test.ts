...
test('MCP exposes strict structured schemas for every tool', () => {
  ...
  for (const definition of toolDefinitions) {
    ...
    assert.ok(
      !('input' in definition.inputSchema.properties) ||
        Object.keys(definition.inputSchema.properties).length > 1,
    );
  }
});
  ...
  const call = await handleMcpMessage(
    {
      ...
      id: 3,
      method: 'tools/call',
      params: { name: 'madeproof_create_project', arguments: { name: 'MCP project' } },
    },
    backend(),
  );
...
test('MCP rejects unknown arguments and unknown tools without executing them', async () => {
  const extra = await handleMcpMessage(
    {
      jsonrpc: '2.0',
      id: 4,
      method: 'tools/call',
      params: { name: 'madeproof_create_project', arguments: { name: 'x', secretOverride: true } },
    },
    backend(),
  );
  ...
  const unknown = await handleMcpMessage(
    {
      ...
      id: 5,
      method: 'tools/call',
      params: { name: 'madeproof_make_it_green', arguments: {} },
    },
    backend(),
  );
