...
try {
  ...
  await api(runtime.url, auth, 'POST', `/runs/${run1.body.id}/evidence`, {
    type: 'TEXT',
    value: { claim: 'Done. Everything works.' },
    source: 'execution-agent',
  });
  const failed = await api<any>(
    runtime.url,
    auth,
    'POST',
    `/runs/${run1.body.id}/verify`,
    {},
    { 'Idempotency-Key': 'e2e-verify-failed' },
  );
  ...
  assert.equal(
    failed.body.results.find((item: any) => item.summary.includes('keyboard'))?.status,
    'FAILED',
  );
  assert.equal(
    failed.body.results.find((item: any) => item.summary.includes('ARIA state'))?.status,
    'FAILED',
  );
  ...
  const verified = await api<any>(
    ...
    auth,
    'POST',
    `/runs/${run2.body.id}/verify`,
    {},
    { 'Idempotency-Key': 'e2e-verify-passed' },
  );
  ...
  const report = {
    ...
    failedRun: {
      id: run1.body.id,
      verdict: failed.body.decision.verdict,
      passed: 5,
      total: 7,
      receiptDigest: failed.body.receipt.digest,
    },
    retryRun: {
      id: run2.body.id,
      verdict: verified.body.decision.verdict,
      passed: 7,
      total: 7,
      receiptDigest: verified.body.receipt.digest,
    },
    evidence: verified.body.results.map((item: any) => ({
