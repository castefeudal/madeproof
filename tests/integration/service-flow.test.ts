...
test('self-reported evidence cannot produce VERIFIED, but independently executed evidence can', async () => {
  ...
  try {
    ...
    const selfTask = service.createTask(actor, { projectId: project.id, title: 'Self report', intent: 'Prove an outcome' });
    service.generateContract(actor, selfTask.id);
    const selfRun = service.startRun(actor, selfTask.id);
    service.addEvidence(actor, selfRun.id, { type: 'JSON', value: { outcomeSatisfied: true, forbiddenAction: false }, source: 'agent' });
    service.addEvidence(actor, selfRun.id, { type: 'TEST_REPORT', value: { status: 'passed' }, source: 'agent' });
    service.addEvidence(actor, selfRun.id, { type: 'COMMAND_OUTPUT', value: { exitCode: 0 }, source: 'agent' });
    const selfVerdict = await service.verify(actor, selfRun.id);
    assert.equal(selfVerdict.decision.verdict, 'FAILED');

    const observedTask = service.createTask(actor, { projectId: project.id, title: 'Observed evidence', intent: 'Prove an outcome' });
...
test('locked contract is immutable and retry creates a new run without rewriting history', async () => {
  ...
  try {
    ...
    const task = service.createTask(actor, { projectId: project.id, title: 'History task', intent: 'Preserve audit history' });
    service.generateContract(actor, task.id);
    const run1 = service.startRun(actor, task.id);
    await service.verify(actor, run1.id);
    assert.throws(() => service.updateContract(actor, task.id, { goal: 'retroactively weakened' }), (error: any) => error.code === 'CONTRACT_LOCKED');
