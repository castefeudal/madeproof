...
export async function startTestApplication(label = 'test', options: { distributed?: boolean } = {}) {
  ...
  if (options.distributed) {
    ...
    worker = new VerificationWorker(workerStore, new EvidenceService(dataDir), path.resolve('.'), started.url, { workerId: `test-worker-${label}`, leaseSeconds: 5, pollIntervalMs: 20 });
