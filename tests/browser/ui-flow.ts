...
try {
  ...
  const hero = await browser.evaluate<string>(
  ...
  assert.match(
    await browser.evaluate<string>(
      `document.querySelector('[data-i18n="heroOne"]')?.textContent || ''`,
    ),
    /AI может сказать/,
  );
  await browser.click('#language-toggle');
  ...
  await browser.waitFor(
    `document.querySelector('#app-shell') && !document.querySelector('#app-shell').classList.contains('hidden')`,
    15000,
  );
  assert.equal(
    await browser.evaluate(`document.querySelector('#route-view h1')?.textContent`),
  ...
  await browser.waitFor(
    `location.pathname.startsWith('/runs/') && document.querySelector('.result-hero h1')?.textContent.trim()==='FAILED'`,
    120000,
    250,
  );
  const failures = await browser.evaluate<string[]>(
    `[...document.querySelectorAll('.failure h3')].map(x=>x.textContent||'')`,
  );
  ...
  await browser.waitFor(
    `location.pathname.startsWith('/runs/') && document.querySelector('.result-hero h1')?.textContent.trim()==='VERIFIED'`,
    120000,
    250,
  );
  const digest = await browser.evaluate<string>(
    `document.querySelector('.receipt-grid div:nth-child(2)')?.textContent?.replace('Digest','').trim() || ''`,
  );
  assert.match(digest, /^[a-f0-9]{64}$/);

  fs.mkdirSync(path.resolve('artifacts/browser'), { recursive: true });
  fs.writeFileSync(
    path.resolve('artifacts/browser/desktop-verified.png'),
  ...
  fs.writeFileSync(
    path.resolve('artifacts/browser/mobile-verified.png'),
    await browser.screenshot(),
  );
  ...
  const report = {
    ...
    desktop: 'artifacts/browser/desktop-verified.png',
    mobile: 'artifacts/browser/mobile-verified.png',
