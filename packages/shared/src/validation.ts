...
export function validateSchema(value: unknown, schema: JsonSchema, path = '$'): void {
  ...
  if (schema.type === 'array') {
    if (!Array.isArray(value))
      throw new MadeProofError('VALIDATION_ERROR', `${path} must be an array`, 422);
    if (schema.items)
  ...
  if (schema.type === 'object') {
    ...
    for (const required of schema.required ?? []) {
      if (!(required in record))
        throw new MadeProofError('VALIDATION_ERROR', `${path}.${required} is required`, 422);
    }
    ...
    if (schema.additionalProperties === false) {
      ...
      for (const key of Object.keys(record)) {
        if (!allowed.has(key))
          throw new MadeProofError('VALIDATION_ERROR', `${path}.${key} is not allowed`, 422);
      }
    }
    return;
  }
  if (schema.type === 'integer') {
    if (!Number.isInteger(value))
      throw new MadeProofError('VALIDATION_ERROR', `${path} must be an integer`, 422);
  ...
  if (typeof value !== schema.type)
    throw new MadeProofError('VALIDATION_ERROR', `${path} must be a ${schema.type}`, 422);
  if (
    schema.type === 'string' &&
