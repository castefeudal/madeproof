...
export class SqliteStore {
  ...
  bootstrapOwner(email: string, passwordHash: string): { userId: string; workspaceId: string } {
    ...
    return this.transaction(() => {
      ...
      this.db
        ...
        .run(workspaceId, 'MADEPROOF Workspace', now);
      this.db
        .prepare(
  ...
  getUserById(id: string): any | null {
    return (
      (this.db.prepare('SELECT id,email,created_at FROM users WHERE id = ?').get(id) as any) ?? null
    );
  }
  ...
  getDefaultWorkspaceForUser(userId: string): any | null {
    return (
      (this.db
        .prepare(
          `SELECT w.*, wm.role FROM workspaces w JOIN workspace_members wm ON wm.workspace_id=w.id WHERE wm.user_id=? ORDER BY w.created_at LIMIT 1`,
        )
  ...
  getSession(tokenHash: string): any | null {
    return (
      (this.db
        .prepare(
          `SELECT s.*, u.email FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.revoked_at IS NULL AND s.expires_at > ?`,
        )
        .get(tokenHash, nowIso()) as any) ?? null
    );
  }
  ...
  createApiKey(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO api_keys(id,workspace_id,created_by,name,prefix,key_hash,scopes_json,expires_at,created_at) VALUES(?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        row.id,
        row.workspaceId,
  ...
  listApiKeys(workspaceId: string): any[] {
    return (
      this.db
        .prepare(
          'SELECT id,name,prefix,scopes_json,expires_at,revoked_at,last_used_at,created_at FROM api_keys WHERE workspace_id=? ORDER BY created_at DESC',
        )
  ...
  createProject(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO projects(id,workspace_id,name,slug,project_type,repository_url,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)`,
      )
      .run(
        id,
        input.workspaceId,
  ...
  createTask(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO tasks(id,workspace_id,project_id,title,intent,template,status,created_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        id,
        input.workspaceId,
  ...
  listTasks(
    ...
    return this.db
      .prepare(
        `SELECT * FROM tasks WHERE ${clauses.join(' AND ')} ORDER BY updated_at DESC LIMIT ? OFFSET ?`,
      )
      .all(...params) as any[];
  }
  ...
  updateTaskStatus(
    ...
    const result = this.db
      .prepare('UPDATE tasks SET status=?,updated_at=? WHERE id=? AND workspace_id=? AND status=?')
      .run(next, nowIso(), taskId, workspaceId, expected);
    if (result.changes !== 1)
      throw new MadeProofError('TASK_STATE_CONFLICT', `Task is no longer in ${expected}`, 409);
  }
  ...
  createContract(workspaceId: string, contract: OutcomeContract): OutcomeContract {
    return this.transaction(() => {
      ...
      this.db
        .prepare(
          `INSERT INTO outcome_contracts(id,workspace_id,task_id,version,goal,expected_outcome,contract_json,created_at,locked_at) VALUES(?,?,?,?,?,?,?,?,?)`,
        )
        .run(
          contract.id,
          workspaceId,
      ...
      const insertCriterion = this.db.prepare(
        `INSERT INTO acceptance_criteria(id,workspace_id,contract_id,position,title,required,severity,category,verification_type,criterion_json) VALUES(?,?,?,?,?,?,?,?,?,?)`,
      );
      ...
      this.db
        .prepare(
          'UPDATE tasks SET latest_contract_version=?,status=?,updated_at=? WHERE id=? AND workspace_id=?',
        )
  ...
  getContract(workspaceId: string, taskId: string, version?: number): OutcomeContract {
    const row = version
      ? this.db
          .prepare(
            'SELECT * FROM outcome_contracts WHERE workspace_id=? AND task_id=? AND version=?',
          )
          .get(workspaceId, taskId, version)
      : this.db
          .prepare(
            'SELECT * FROM outcome_contracts WHERE workspace_id=? AND task_id=? ORDER BY version DESC LIMIT 1',
          )
  ...
  listContracts(workspaceId: string, taskId: string): OutcomeContract[] {
    ...
    return (
      this.db
        .prepare(
          'SELECT contract_json,locked_at FROM outcome_contracts WHERE workspace_id=? AND task_id=? ORDER BY version DESC',
        )
        .all(workspaceId, taskId) as any[]
    ).map((row) => ({
      ...parseJson<OutcomeContract>(row.contract_json, {} as OutcomeContract),
  ...
  lockContract(workspaceId: string, contractId: string): string {
    ...
    this.transaction(() => {
      const row = this.db
        .prepare(
          'SELECT contract_json,locked_at FROM outcome_contracts WHERE id=? AND workspace_id=?',
        )
        .get(contractId, workspaceId) as any;
      if (!row) throw new MadeProofError('CONTRACT_NOT_FOUND', 'Outcome contract not found', 404);
      if (!row.locked_at) {
        ...
        this.db
          .prepare(
            'UPDATE outcome_contracts SET locked_at=?,contract_json=? WHERE id=? AND workspace_id=?',
          )
  ...
  startRun(input: {
    ...
    return this.transaction(() => {
      ...
      const contractRow = this.db
        ...
        .get(contract.id, input.workspaceId) as any;
      if (!contractRow)
        throw new MadeProofError('CONTRACT_NOT_FOUND', 'Outcome contract not found', 404);
      if (!contractRow.locked_at) {
        const lockedAt = nowIso();
        const lockedContract = parseJson<OutcomeContract>(
          contractRow.contract_json,
          {} as OutcomeContract,
        );
      ...
      const last = this.db
        .prepare('SELECT MAX(attempt) AS attempt FROM runs WHERE task_id=?')
      ...
      this.db
        .prepare(
          `INSERT INTO runs(id,workspace_id,task_id,contract_id,contract_version,status,attempt,agent_id,artifact_ref,metadata_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
        )
        .run(
          id,
          input.workspaceId,
  ...
  listRuns(workspaceId: string, taskId: string): any[] {
    ...
    return (
      this.db
        .prepare('SELECT * FROM runs WHERE workspace_id=? AND task_id=? ORDER BY attempt DESC')
        .all(workspaceId, taskId) as any[]
  ...
  updateRunStatus(workspaceId: string, runId: string, expected: RunStatus, next: RunStatus): void {
    ...
    const result = this.db
      .prepare(
        `UPDATE runs SET status=?,updated_at=?,started_at=COALESCE(started_at,?),finished_at=COALESCE(?,finished_at) WHERE id=? AND workspace_id=? AND status=?`,
      )
      .run(next, now, started, finished, runId, workspaceId, expected);
    if (result.changes !== 1)
      throw new MadeProofError('RUN_STATE_CONFLICT', `Run is no longer in ${expected}`, 409);
  }
  ...
  addEvidence(item: EvidenceItem): EvidenceItem {
    ...
    this.db
      .prepare(
        `INSERT INTO evidence(id,workspace_id,run_id,criterion_id,type,source,source_actor,created_at,observed_at,content_hash,mime_type,size_bytes,storage_location,provenance,trust_tier,sanitization_state,value_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        item.id,
        item.workspaceId,
        ...
        item.provenance,
        item.trustTier,
        item.sanitizationState,
        item.value === undefined ? null : JSON.stringify(item.value),
      );
    return item;
  }
  ...
  listEvidence(workspaceId: string, runId: string): EvidenceItem[] {
    ...
    return (
      this.db
        .prepare('SELECT * FROM evidence WHERE workspace_id=? AND run_id=? ORDER BY created_at')
  ...
  upsertCheck(input: {
    ...
    const existing = this.db
      .prepare('SELECT * FROM verification_checks WHERE run_id=? AND criterion_id=?')
      .get(input.runId, input.criterionId) as any;
    if (existing) return existing;
    const id = newId('chk');
    const now = nowIso();
    this.db
      .prepare(
        `INSERT INTO verification_checks(id,workspace_id,run_id,criterion_id,type,status,config_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        id,
        input.workspaceId,
  ...
  setCheckStatus(workspaceId: string, checkId: string, status: string): void {
    const result = this.db
      .prepare('UPDATE verification_checks SET status=?,updated_at=? WHERE id=? AND workspace_id=?')
      .run(status, nowIso(), checkId, workspaceId);
    if (result.changes !== 1)
  ...
  saveResult(workspaceId: string, runId: string, result: VerificationResult): void {
    this.transaction(() => {
      this.db
        .prepare(
          `INSERT OR REPLACE INTO verification_results(id,workspace_id,run_id,check_id,criterion_id,status,started_at,finished_at,duration_ms,summary,details_json,evidence_ids_json,confidence,error_code,error_message) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        )
        .run(
          result.id,
          workspaceId,
  ...
  getResults(workspaceId: string, runId: string): VerificationResult[] {
    ...
    return (
      this.db
        .prepare(
          'SELECT * FROM verification_results WHERE workspace_id=? AND run_id=? ORDER BY started_at',
        )
        .all(workspaceId, runId) as any[]
    ).map((row) => ({
      id: row.id,
  ...
  saveVerdict(workspaceId: string, runId: string, decision: VerdictDecision): any {
    ...
    this.db
      .prepare(
        `INSERT INTO verdicts(id,workspace_id,run_id,machine_verdict,confidence,reason,decision_json,created_at) VALUES(?,?,?,?,?,?,?,?)`,
      )
      .run(
        row.id,
        workspaceId,
  ...
  saveReceipt(input: {
    ...
    const existing = this.db
      .prepare('SELECT * FROM receipts WHERE run_id=? AND workspace_id=?')
      .get(input.runId, input.workspaceId) as any;
    if (existing) return this.getReceipt(input.workspaceId, existing.id);
    const id = newId('rcp');
    const createdAt = nowIso();
    this.db
      .prepare(
        `INSERT INTO receipts(id,workspace_id,run_id,receipt_version,receipt_json,digest,signature,signing_key_id,created_at) VALUES(?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        id,
        input.workspaceId,
  ...
  getReceipt(workspaceId: string, receiptId: string): any {
    const row = this.db
      .prepare('SELECT * FROM receipts WHERE id=? AND workspace_id=?')
      .get(receiptId, workspaceId) as any;
  ...
  getReceiptByRun(workspaceId: string, runId: string): any {
    const row = this.db
      .prepare('SELECT id FROM receipts WHERE run_id=? AND workspace_id=?')
      .get(runId, workspaceId) as any;
    if (!row) throw new MadeProofError('RECEIPT_NOT_FOUND', 'Verification receipt not found', 404);
  ...
  appendAudit(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO audit_events(id,workspace_id,actor_id,actor_type,action,resource_type,resource_id,timestamp,previous_state_digest,resulting_state_digest,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        newId('aud'),
        input.workspaceId,
        ...
        input.resourceId,
        nowIso(),
        input.previous === undefined ? null : sha256(canonicalJson(input.previous)),
        input.resulting === undefined ? null : sha256(canonicalJson(input.resulting)),
        JSON.stringify(input.metadata ?? {}),
      );
  }
  ...
  getIdempotency(workspaceId: string, key: string, route: string): any | null {
    const row = this.db
      .prepare(
        'SELECT * FROM idempotency_records WHERE workspace_id=? AND key=? AND route=? AND expires_at>?',
      )
      .get(workspaceId, key, route, nowIso()) as any;
  ...
  saveIdempotency(input: {
    ...
    this.db
      .prepare(
        `INSERT OR REPLACE INTO idempotency_records(workspace_id,key,route,request_hash,response_status,response_json,created_at,expires_at) VALUES(?,?,?,?,?,?,?,?)`,
      )
      .run(
        input.workspaceId,
        input.key,
  ...
  listAttention(workspaceId: string): any[] {
    return this.db
      .prepare(
        `SELECT t.*, p.name AS project_name FROM tasks t JOIN projects p ON p.id=t.project_id WHERE t.workspace_id=? AND t.status IN ('FAILED','REVIEW_REQUIRED') ORDER BY t.updated_at DESC LIMIT 50`,
      )
      .all(workspaceId) as any[];
  }
