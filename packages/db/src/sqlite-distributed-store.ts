...
function parse<T>(v: unknown, f: T): T {
  if (v == null) return f;
  if (typeof v === 'string') return JSON.parse(v) as T;
  return v as T;
}
export class SqliteDistributedStore extends SqliteStore {
  private readonly q: any;
  constructor(readonly distributedDataDir: string) {
    super(distributedDataDir);
    this.q = new DatabaseSync(path.join(distributedDataDir, 'madeproof.db'));
    this.q.exec('PRAGMA foreign_keys = ON; PRAGMA journal_mode = WAL; PRAGMA busy_timeout = 5000;');
  }
  override migrate() {
    super.migrate();
    const f = path.resolve('packages/db/migrations/sqlite/0002_worker_runner.sql');
    if (fs.existsSync(f)) this.q.exec(fs.readFileSync(f, 'utf8'));
  }
  override close() {
    try {
      this.q.close();
  ...
  enqueueVerificationJob(i: VerificationJobInput) {
    ...
    return this.q.prepare('SELECT * FROM verification_jobs WHERE id=?').get(id);
  }
  ...
  claimVerificationJob(workerId: string, leaseSeconds: number) {
    ...
    try {
      ...
      const o = this.q.prepare('SELECT * FROM verification_jobs WHERE id=?').get(r.id);
      this.q.exec('COMMIT');
      return o;
    } catch (e) {
      this.q.exec('ROLLBACK');
      throw e;
    }
  }
  ...
  getRunnerJob(id: string) {
    const r = this.q.prepare('SELECT * FROM runner_jobs WHERE id=?').get(id) as any;
    return r
      ? {
          ...r,
          payload: parse(r.payload_json, {}),
          result: parse(r.result_json, null),
          error: parse(r.error_json, null),
        }
      : null;
  }
