...
export function hashPassword(password: string): string {
  if (password.length < 10) throw new MadeProofError('WEAK_PASSWORD', 'Password must contain at least 10 characters', 422);
...
export function encryptSecret(plaintext: string, masterKey: string, keyVersion = 1): string {
  ...
  return JSON.stringify({ v: keyVersion, iv: iv.toString('base64url'), tag: tag.toString('base64url'), data: ciphertext.toString('base64url') });
}
...
export function decryptSecret(payload: string, masterKey: string): string {
  ...
  const decipher = crypto.createDecipheriv('aes-256-gcm', keyFromSecret(masterKey), Buffer.from(parsed.iv, 'base64url'));
