...
function result(input: {
  ...
  return {
    ...
    confidence:
      input.confidence ?? (input.status === 'PASSED' || input.status === 'FAILED' ? 1 : 0),
    errorCode: input.errorCode,
    errorMessage: input.errorMessage,
  };
}

function findTrustedEvidence(
  ...
  return evidence.filter(
    (item) =>
      item.trustTier >= 2 &&
      (!required.size || required.has(item.type)) &&
      (!item.criterionId || item.criterionId === criterion.id),
  );
}
...
export class EvidenceMatchCheck implements VerificationCheck {
  ...
  async execute(
    ...
    const items = findTrustedEvidence(context.evidence, criterion).filter(
      (item) => !expected.type || item.type === expected.type,
    );
    ...
    return result({
      ...
      details: {
        expected,
        consideredEvidence: items.map((item) => ({
          id: item.id,
  ...
  async execute(
    ...
    try {
      ...
      return result({
        ...
        startedAt,
        summary: passed
          ? `${this.type} completed with exit code 0.`
          : `${this.type} did not complete successfully.`,
        details: {
          command,
          args,
...
async function openDemoDocument(
  ...
  if (!htmlResponse.ok || !cssResponse.ok || !scriptResponse.ok) {
    throw new Error(
      `Demo target could not be fetched: html=${htmlResponse.status} css=${cssResponse.status} js=${scriptResponse.status}`,
    );
  }
  const [rawHtml, css, script] = await Promise.all([
    htmlResponse.text(),
    cssResponse.text(),
    scriptResponse.text(),
  ]);
  ...
  await browser.evaluate(
    `globalThis.__MADEPROOF_DEMO_FIXED__=${Boolean(context.run.metadata?.demoFixed) ? 'true' : 'false'}`,
  );
...
//# sourceURL=madeproof-demo-target.js
  ...
  return {
    ...
    reason:
      'The managed Chromium image blocks all navigated URLs by administrator policy; MADEPROOF still verifies real Chromium DOM, pointer and keyboard behavior after independently fetching the served target.',
  };
}
...
export class BrowserInteractionCheck implements VerificationCheck {
  ...
  async execute(
    ...
    try {
      ...
      if (mode === 'responsive') {
        const widths = Array.isArray(criterion.expected.widths)
          ? criterion.expected.widths.map(Number)
          : [320, 375, 390, 430];
        const checks: Array<{
          width: number;
          overflow: boolean;
      ...
      observed = await browser.evaluate<Record<string, unknown>>(
        `(() => { const b=document.querySelector('#scenario-b'); return {active:document.body.dataset.active,ariaPressed:b?.getAttribute('aria-pressed'),focused:document.activeElement?.id}; })()`,
      );
      ...
      return result({
        ...
        startedAt,
        summary: passed
          ? `Scenario ${criterion.expected.active} activated through ${mode}.`
          : `Scenario ${criterion.expected.active} did not activate through ${mode}.`,
        details: {
...
export class AriaSnapshotCheck implements VerificationCheck {
  ...
  async execute(
    ...
    try {
      ...
      await browser.focus('#scenario-b');
      await browser.press('Enter');
      const snapshot = await browser.evaluate<Record<string, unknown>>(
        `(() => { const el=document.querySelector('#scenario-b'); return {role:el?.getAttribute('role')||el?.tagName.toLowerCase(),name:el?.textContent?.trim(),ariaPressed:el?.getAttribute('aria-pressed'),active:document.body.dataset.active}; })()`,
      );
      snapshot.browserTransport = browserTransport;
      const ariaEvidence = context.evidenceService.createInline({
        workspaceId: context.workspaceId,
        runId: context.run.id,
        criterionId: criterion.id,
        type: 'ARIA_SNAPSHOT',
      ...
      return result({
        ...
        details: { expected: criterion.expected, observed: snapshot },
        evidenceIds: [ariaEvidence.id],
      });
    } catch (error) {
      return result({
        checkId,
  ...
  async execute(
    ...
    try {
      ...
      const evidence = context.evidenceService.createInline({
        ...
        criterionId: criterion.id,
        type: 'JSON',
        value: auditWithTransport,
        source: 'chromium-cdp-accessibility-smoke',
        sourceActor: 'madeproof-runner',
        provenance: 'EXECUTED_BY_MADEPROOF',
      });
      context.store.addEvidence(evidence);
...
export class FileExistsCheck implements VerificationCheck {
  ...
  async execute(
    ...
    return result({
      ...
      details: { path: relative, safe, exists },
    });
  }
}
  ...
  async execute(
    ...
    try {
      ...
      return result({
        ...
        details: { status: response.status, expectedStatus, bytes: bytes.byteLength },
      });
    } catch (error) {
      return result({
        checkId,
...
export class ManualReviewCheck implements VerificationCheck {
  ...
  async execute(
    ...
    return result({
      ...
      status: 'INCONCLUSIVE',
      startedAt: nowIso(),
      summary: 'This criterion requires human review.',
      confidence: 0,
    });
  }
}
