...
export function generateOutcomeContract(input: {
  ...
  const criteria = isDemo
    ? [
        criterion(
          1,
          'Scenario B activates on pointer click',
          'functional',
          'browser_interaction',
          { mode: 'pointer', active: 'B' },
          ['SCREENSHOT', 'TRACE'],
        ),
        criterion(
          2,
          'Scenario B activates using keyboard',
          'accessibility',
          'browser_interaction',
          { mode: 'keyboard', active: 'B' },
          ['SCREENSHOT', 'TRACE'],
        ),
        criterion(
          3,
          'aria-pressed reflects the active scenario',
          'accessibility',
          'aria_snapshot',
          { target: 'scenario-b', value: 'true' },
          ['ARIA_SNAPSHOT'],
        ),
        criterion(
          4,
          ...
          {
            command: 'node',
            args: ['--test', 'examples/demo-target/test.mjs'],
            network: 'disabled',
          },
          ['TEST_REPORT'],
        ),
        criterion(
          ...
          'build',
          'build',
          { command: 'node', args: ['examples/demo-target/build.mjs'], network: 'disabled' },
          ['COMMAND_OUTPUT'],
        ),
        criterion(
          6,
          ...
          'functional',
          'browser_interaction',
          { mode: 'responsive', widths: [320, 375, 390, 430] },
          ['SCREENSHOT'],
        ),
        criterion(
          7,
          'No new critical accessibility defects are detected',
          'accessibility',
          'accessibility',
          { seriousOrCritical: 0 },
          ['JSON'],
    ...
    : [
        criterion(
          1,
          'Requested outcome is observable in the submitted artifact',
          'functional',
          'evidence_match',
          { type: 'JSON', field: 'outcomeSatisfied', equals: true },
          ['JSON'],
        ),
        criterion(
          2,
          ...
          'test',
          'evidence_match',
          { type: 'TEST_REPORT', status: 'passed' },
          ['TEST_REPORT'],
        ),
        criterion(
          3,
          ...
          'build',
          'evidence_match',
          { type: 'COMMAND_OUTPUT', exitCode: 0 },
          ['COMMAND_OUTPUT'],
        ),
        criterion(
          4,
          'No forbidden action was reported or observed',
          'policy',
          'evidence_match',
          { type: 'JSON', field: 'forbiddenAction', equals: false },
          ['JSON'],
        ),
      ];
  ...
  return {
    ...
    forbiddenActions: [
      'Falsifying evidence',
      'Changing a locked contract retroactively',
      'Using agent self-report as final proof',
    ],
    requiredEvidence: [...new Set(criteria.flatMap((item) => item.evidenceRequirements))],
    risk: {
      level: isDemo ? 'medium' : 'low',
      rationale: isDemo
        ? 'User interaction and accessibility can regress independently.'
