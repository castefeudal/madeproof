...
export class MadeProofService {
  ...
  login(email: string, password: string): { token: string; csrfToken: string; expiresAt: string; user: any; workspace: any } {
  ...
  authenticateApiKey(token: string, requiredScope?: string): Actor {
    ...
    if (requiredScope && !key.scopes.includes(requiredScope) && !key.scopes.includes('*')) {
      throw new MadeProofError('INSUFFICIENT_SCOPE', `API key requires scope ${requiredScope}`, 403);
    }
    return { id: key.id, workspaceId: key.workspace_id, type: 'API_KEY', scopes: key.scopes };
  }

  createApiKey(actor: Actor, input: { name: string; scopes: string[]; expiresAt?: string }): { secret: string; key: any } {
    ...
    const key = this.store.createApiKey({ workspaceId: actor.workspaceId, userId: actor.id, name: input.name, prefix, keyHash: hashToken(secret), scopes, expiresAt: input.expiresAt });
    this.audit(actor, 'api_key.created', 'api_key', key.id, undefined, { name: input.name, scopes });
    return { secret, key: { id: key.id, name: key.name, prefix: key.prefix, scopes: key.scopes, expiresAt: key.expiresAt, createdAt: key.createdAt } };
  }
  ...
  createProject(actor: Actor, input: { name: string; projectType?: string; repositoryUrl?: string }): any {
  ...
  createTask(actor: Actor, input: { projectId: string; title: string; intent: string; template?: string }): any {
    this.requireScope(actor, 'tasks:write');
    const task = this.store.createTask({ workspaceId: actor.workspaceId, actorId: actor.id, ...input });
    this.audit(actor, 'task.created', 'task', task.id, undefined, task);
    return task;
  }

  listTasks(actor: Actor, filters: { status?: string; projectId?: string; limit?: number; offset?: number } = {}): any[] {
  ...
  generateContract(actor: Actor, taskId: string): OutcomeContract {
    ...
    const contract = generateOutcomeContract({ taskId, version, intent: task.intent, template: task.template ?? undefined });
  ...
  updateContract(actor: Actor, taskId: string, input: Partial<OutcomeContract> & { acceptanceCriteria?: AcceptanceCriterion[] }): OutcomeContract {
    this.requireScope(actor, 'tasks:write');
    const task = this.store.getTask(actor.workspaceId, taskId);
    const previous = this.store.getContract(actor.workspaceId, taskId);
    if (previous.lockedAt) throw new MadeProofError('CONTRACT_LOCKED', 'This contract version is attached to a run. Create a new task or retry the locked contract.', 409);
    ...
    if (!next.acceptanceCriteria.length) throw new MadeProofError('VALIDATION_ERROR', 'Outcome contract requires at least one acceptance criterion', 422);
  ...
  startRun(actor: Actor, taskId: string, input: { metadata?: Record<string, unknown>; artifactRef?: string; agentId?: string } = {}): any {
    this.requireScope(actor, 'tasks:write');
    const task = this.store.getTask(actor.workspaceId, taskId);
    if (!['READY', 'FAILED', 'REVIEW_REQUIRED', 'VERIFIED'].includes(task.status)) throw new MadeProofError('TASK_NOT_RUNNABLE', `Task in ${task.status} cannot start a new run`, 409);
    ...
    const run = this.store.startRun({ workspaceId: actor.workspaceId, taskId, actorId: actor.id, ...input });
    this.audit(actor, 'run.created', 'run', run.id, undefined, run);
    return run;
  }

  addEvidence(actor: Actor, runId: string, input: { criterionId?: string; type: string; value: unknown; source?: string; mimeType?: string }): any {
    this.requireScope(actor, 'evidence:write');
    const run = this.store.getRun(actor.workspaceId, runId);
    if (!['AWAITING_EVIDENCE', 'RUNNING'].includes(run.status)) throw new MadeProofError('RUN_NOT_ACCEPTING_EVIDENCE', `Run in ${run.status} is not accepting evidence`, 409);
    ...
    this.audit(actor, 'evidence.added', 'evidence', item.id, undefined, { runId, type: item.type, provenance: item.provenance, hash: item.contentHash });
    return item;
  }
  ...
  retry(actor: Actor, runId: string, input: { metadata?: Record<string, unknown>; artifactRef?: string } = {}): any {
    this.requireScope(actor, 'verification:run');
    const previous = this.store.getRun(actor.workspaceId, runId);
    return this.startRun(actor, previous.task_id, { metadata: { ...previous.metadata, ...input.metadata }, artifactRef: input.artifactRef ?? previous.artifact_ref });
  }
  ...
  getVerification(actor: Actor, runId: string): any {
    this.requireScope(actor, 'tasks:read');
    return { results: this.store.getResults(actor.workspaceId, runId), verdict: this.store.getVerdict(actor.workspaceId, runId) };
  }
  ...
  getFailedChecks(actor: Actor, runId: string): any[] {
    ...
    const run = this.store.getRun(actor.workspaceId, runId);
    const contract = this.store.getContract(actor.workspaceId, run.task_id, run.contract_version);
    return results.filter((item) => ['FAILED', 'INCONCLUSIVE', 'ERROR'].includes(item.status)).map((item) => ({ ...item, criterion: contract.acceptanceCriteria.find((criterion) => criterion.id === item.criterionId) }));
  }
  ...
  dashboard(actor: Actor): any {
    this.requireScope(actor, 'tasks:read');
    return { counts: this.store.dashboardCounts(actor.workspaceId), attention: this.store.listAttention(actor.workspaceId), projects: this.store.listProjects(actor.workspaceId, 10, 0), tasks: this.store.listTasks(actor.workspaceId, { limit: 20 }) };
  }

  agentReliability(actor: Actor, agentId?: string): any {
    this.requireScope(actor, 'tasks:read');
    return { agentId: agentId ?? null, status: 'INSUFFICIENT_SAMPLE', minimumSample: 5, message: 'MADEPROOF does not calculate reliability from fewer than five completed runs in the same domain.' };
  }

  idempotent<T>(actor: Actor, key: string | undefined, route: string, input: unknown, operation: () => { status: number; body: T }): { status: number; body: T; replayed: boolean } {
    if (!key) {
    ...
    const value = operation();
    this.store.saveIdempotency({ workspaceId: actor.workspaceId, key, route, requestHash, responseStatus: value.status, response: value.body });
    return { ...value, replayed: false };
  }
  ...
  private requireScope(actor: Actor, scope: string): void {
    ...
    if (!actor.scopes.includes(scope) && !readAlias) throw new MadeProofError('INSUFFICIENT_SCOPE', `This operation requires scope ${scope}`, 403);
  }

  private requireUser(actor: Actor): void {
    if (actor.type !== 'USER') throw new MadeProofError('USER_SESSION_REQUIRED', 'This operation requires an interactive user session', 403);
  }

  private audit(actor: Actor, action: string, resourceType: string, resourceId: string, previous?: unknown, resulting?: unknown): void {
    this.store.appendAudit({ workspaceId: actor.workspaceId, actorId: actor.id, actorType: actor.type, action, resourceType, resourceId, previous, resulting, metadata: {} });
  }
}
