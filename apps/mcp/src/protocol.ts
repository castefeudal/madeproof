...
export const toolDefinitions = [
  tool(
    'madeproof_create_project',
    'Create a MADEPROOF project.',
    object({ name: string(), projectType: optional(string()), repositoryUrl: optional(string()) }, [
      'name',
    ]),
  ),
  tool(
    'madeproof_create_task',
    'Create a verification task from human intent.',
    object(
      { projectId: string(), title: string(), intent: string(), template: optional(string()) },
      ['projectId', 'title', 'intent'],
    ),
  ),
  tool(
    'madeproof_generate_contract',
    'Generate an observable Outcome Contract.',
    object({ taskId: string() }, ['taskId']),
  ),
  tool(
    'madeproof_get_contract',
    'Get the latest Outcome Contract.',
    object({ taskId: string() }, ['taskId']),
  ),
  tool(
    'madeproof_update_contract',
    'Create a new contract version from an edited contract payload.',
    object({ taskId: string(), contract: { type: 'object' } }, ['taskId', 'contract']),
  ),
  tool(
    'madeproof_start_run',
    'Lock the contract and start a run.',
    object(
      {
        taskId: string(),
  ...
  tool(
    'madeproof_add_evidence',
    'Submit evidence. Agent evidence remains self-reported until independently observed.',
    object(
      {
        runId: string(),
        criterionId: optional(string()),
        type: string(),
        value: {},
  ...
  tool(
    'madeproof_verify',
    'Run independent verification for a run.',
    object({ runId: string() }, ['runId']),
  ),
  tool(
    'madeproof_get_verdict',
    'Get the latest machine verdict.',
    object({ runId: string() }, ['runId']),
  ),
  tool(
    'madeproof_get_failed_checks',
    'Get failed, inconclusive, or errored checks.',
    object({ runId: string() }, ['runId']),
  ),
  tool(
    'madeproof_get_required_actions',
    'Get concrete next actions from unresolved checks.',
    object({ runId: string() }, ['runId']),
  ),
  tool(
    'madeproof_retry',
    'Create a new immutable retry run.',
    object(
      { runId: string(), metadata: optional({ type: 'object' }), artifactRef: optional(string()) },
      ['runId'],
    ),
  ),
  tool(
    'madeproof_get_receipt',
    'Get the immutable logical verification receipt.',
    object({ runId: string() }, ['runId']),
  ),
  tool('madeproof_list_attention', 'List tasks that need human attention.', object({}, [])),
  tool(
    'madeproof_get_agent_reliability',
    'Get contextual reliability only when sample size is sufficient.',
    object({ agentId: optional(string()) }, []),
  ),
];
...
function object(properties: Record<string, any>, required: string[]): any {
  return { type: 'object', properties, required, additionalProperties: false };
}
function tool(name: string, description: string, inputSchema: any): any {
...
function assertArguments(name: string, args: any): void {
  ...
  if (!args || typeof args !== 'object' || Array.isArray(args))
    throw new MadeProofError('MCP_INVALID_ARGUMENTS', 'Tool arguments must be an object', 422);
  const allowed = new Set(Object.keys(definition.inputSchema.properties));
  for (const required of definition.inputSchema.required)
    if (!(required in args))
      throw new MadeProofError('MCP_INVALID_ARGUMENTS', `${required} is required`, 422);
  for (const key of Object.keys(args))
    if (!allowed.has(key))
      throw new MadeProofError('MCP_INVALID_ARGUMENTS', `${key} is not allowed`, 422);
}

async function callTool(backend: ToolBackend, name: string, args: any): Promise<any> {
  assertArguments(name, args);
  switch (name) {
    case 'madeproof_create_project':
      return backend.createProject(args);
    case 'madeproof_create_task':
      return backend.createTask(args);
    case 'madeproof_generate_contract':
      return backend.generateContract(args.taskId);
    case 'madeproof_get_contract':
      return backend.getContract(args.taskId);
    case 'madeproof_update_contract':
      return backend.updateContract(args.taskId, args.contract);
    case 'madeproof_start_run':
      return backend.startRun(args.taskId, args);
    case 'madeproof_add_evidence':
      return backend.addEvidence(args.runId, args);
    case 'madeproof_verify':
      return backend.verify(args.runId);
    case 'madeproof_get_verdict':
      return backend.getVerdict(args.runId);
    case 'madeproof_get_failed_checks':
      return backend.getFailedChecks(args.runId);
    case 'madeproof_get_required_actions': {
      const failed = await backend.getFailedChecks(args.runId);
      return failed.map((item) => ({
        criterionId: item.criterionId,
        criterion: item.criterion?.title,
        status: item.status,
        expected: item.criterion?.expected,
        observed: item.details,
        nextAction:
          item.status === 'ERROR'
            ? 'Restore verifier infrastructure and retry.'
            : item.status === 'INCONCLUSIVE'
              ? 'Provide stronger evidence or complete human review.'
              : 'Fix the observed mismatch, create a retry run, and verify again.',
      }));
    }
    case 'madeproof_retry':
      return backend.retry(args.runId, args);
    case 'madeproof_get_receipt':
      return backend.getReceipt(args.runId);
    case 'madeproof_list_attention':
      return backend.listAttention();
    case 'madeproof_get_agent_reliability':
      return backend.getAgentReliability(args.agentId);
    default:
      throw new MadeProofError('MCP_TOOL_NOT_FOUND', `Unknown MCP tool: ${name}`, 404);
  }
}
    ...
    if (message.method === 'server/discover') {
      return {
        ...
        result: {
          ...
          serverInfo: { name: 'madeproof', version: '0.1.0' },
          serverCapabilities: { tools: { listChanged: false }, tasks: { polling: true } },
        },
      };
    }
...
export async function handleMcpHttp(input: {
  request: any;
  response: any;
  ...
  if (origin && origin !== new URL(input.config.publicBaseUrl).origin) {
    return sendJson(input.response, 403, {
      jsonrpc: '2.0',
      error: { code: -32001, message: 'Origin rejected' },
    });
  }
  ...
  const nameHeader = input.request.headers['mcp-name']
    ? decodeHeader(String(input.request.headers['mcp-name']))
    : undefined;
  ...
  if (protocol !== PROTOCOL_VERSION || bodyVersion !== protocol) {
    return sendJson(input.response, 400, {
      ...
      error: {
        code: -32020,
        message: 'Header mismatch: unsupported or inconsistent MCP protocol version',
  ...
  if (
    ...
    (body.method === 'tools/call' && nameHeader !== body.params?.name)
  ) {
    return sendJson(input.response, 400, {
      jsonrpc: '2.0',
      id: body?.id ?? null,
