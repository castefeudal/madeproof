...
class CdpClient {
  ...
  static async connect(url: string): Promise<CdpClient> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      socket.addEventListener('error', () => reject(new Error('Could not connect to Chromium DevTools Protocol')), { once: true });
    });
...
async function waitForJson(url: string, timeoutMs = 10000): Promise<any> {
  ...
  throw new MadeProofError('BROWSER_START_FAILED', `Chromium debugging endpoint did not become ready: ${String(lastError ?? '')}`, 500);
}
...
export class BrowserSession {
  ...
  async start(width = 1280, height = 900): Promise<void> {
    ...
    this.processHandle = childProcess.spawn(chromium, [
      '--headless=new',
      '--no-sandbox',
    ...
    let launchError = '';
    this.processHandle.stderr.on('data', (chunk: any) => { launchError += String(chunk); });
    ...
    if (!page?.webSocketDebuggerUrl) throw new MadeProofError('BROWSER_START_FAILED', `No Chromium page target found. ${launchError}`, 500);
    ...
    this.client.on('Runtime.exceptionThrown', (params) => this.consoleErrors.push(params.exceptionDetails?.text ?? 'Unhandled exception'));
    this.client.on('Runtime.consoleAPICalled', (params) => {
      if (params.type === 'error') this.consoleErrors.push(params.args?.map((arg: any) => arg.value ?? arg.description).join(' ') ?? 'console.error');
    });
  ...
  async setViewport(width: number, height = 900): Promise<void> {
    await this.client.send('Emulation.setDeviceMetricsOverride', { width, height, deviceScaleFactor: 1, mobile: width < 600 });
  }
  ...
  async navigate(url: string): Promise<void> {
    ...
    await Promise.race([loaded, new Promise((_, reject) => setTimeout(() => reject(new Error('Navigation timeout')), 10000))]);
    await new Promise((resolve) => setTimeout(resolve, 100));
  }

  async evaluate<T>(expression: string): Promise<T> {
    const result = await this.client.send('Runtime.evaluate', { expression, returnByValue: true, awaitPromise: true });
  ...
  async setDocumentContent(html: string): Promise<void> {
    ...
    if (!frameId) throw new MadeProofError('BROWSER_DOCUMENT_FAILED', 'Chromium did not expose a main frame', 500);
  ...
  async click(selector: string): Promise<void> {
    const point = await this.evaluate<{ x: number; y: number }>(`(() => { const el=document.querySelector(${JSON.stringify(selector)}); if(!el) throw new Error('Element not found'); const r=el.getBoundingClientRect(); return {x:r.left+r.width/2,y:r.top+r.height/2}; })()`);
  ...
  async type(selector: string, text: string): Promise<void> {
    await this.evaluate(`(() => { const el=document.querySelector(${JSON.stringify(selector)}); if(!el) throw new Error('Element not found'); el.focus(); if('value' in el) el.value=''; })()`);
    await this.client.send('Input.insertText', { text });
    await this.evaluate(`(() => { const el=document.querySelector(${JSON.stringify(selector)}); if(el){ el.dispatchEvent(new Event('input',{bubbles:true})); el.dispatchEvent(new Event('change',{bubbles:true})); } })()`);
  }
  ...
  async waitFor<T>(expression: string, timeoutMs = 15000, intervalMs = 100): Promise<T> {
    ...
    throw new MadeProofError('BROWSER_WAIT_TIMEOUT', `Browser condition did not become true within ${timeoutMs} ms`, 500, { expression, last });
  }
  ...
  async press(key: 'Enter' | 'Space'): Promise<void> {
    ...
    await this.client.send('Input.dispatchKeyEvent', { type: 'keyDown', key: text, code, windowsVirtualKeyCode: key === 'Space' ? 32 : 13, nativeVirtualKeyCode: key === 'Space' ? 32 : 13 });
    ...
    await this.client.send('Input.dispatchKeyEvent', { type: 'keyUp', key: text, code, windowsVirtualKeyCode: key === 'Space' ? 32 : 13, nativeVirtualKeyCode: key === 'Space' ? 32 : 13 });
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  ...
  async close(): Promise<void> {
    ...
    try { fs.rmSync(this.profileDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); } catch {}
  }
}
