...
if (!(await app.service.listProjects(actor)).length) {
  ...
  const task = await app.service.createTask(actor, { projectId: project.id, title: 'Fix Scenario B interaction', intent: 'Fix Scenario B selector interaction. It must work with mouse and keyboard without breaking accessibility state.', template: 'frontend-bug-fix-demo' });
  await app.service.generateContract(actor, task.id);
}
await app.service.store.close();
